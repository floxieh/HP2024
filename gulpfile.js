...
// function devImages() {
//   return src(`${options.paths.src.encarts}/**/*`)
//   .pipe(dest('dist/')
//   );
// }

function devImages() {
  return src(`${options.paths.src.encarts}/**/*`).pipe(
    dest("dist/encarts/"),
  );
}

function devImages2() {
  return src(`${options.paths.src.img}/**`).pipe(dest("dist/img/"));
}
...
function watchFiles() {
  ...
  watch(
    `${options.paths.src.encarts}/*`,
    series(devImages, previewReload),
  );
  watch(`${options.paths.src.img}/*`, series(devImages2, previewReload));
...
function prodHTML() {
  return src(`${options.paths.src.base}/MEL_SXX.html`)
    .pipe(includePartials())
    .pipe(dest("build/MEL_READY"));
}
...
exports.prod = series(
  ...
  parallel(prodStyles, prodImages, prodHTML, prodFonts, prodThirdParty), //Run All tasks in parallel
  buildFinish,
);
